...
M1 <- read.table("stability.TETFecalControl.0.03.Top30.pearson.otu.corr", header = TRUE, sep="\t", as.is=T)
...
## Column 4 is the p-value of the correlation.
## If the p-value of a particular correlation is above 0.05 then it is disregarded.
M2 <- read.table("stability.TETFecalExperimental.0.03.Top30.pearson.otu.corr", header = TRUE, sep="\t", as.is=T)
M2 <- ifelse(M2$Significance>0.05,NA,M2$pearsonCoef)

## Compute the F-statistical test comparing the two correlation vectors
