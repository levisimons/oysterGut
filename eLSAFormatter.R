...
## Read in eLSA input file.
LSAInput <- read.table("eLSAMicrobiome.tsv", header=FALSE, sep="\t",as.is=T)

## Now to read in data generated by eLSA.
LSAOutput <- read.table("LSAOutput.txt", header=TRUE, sep="\t",as.is=T)
LSASubset <- LSAOutput[LSAOutput$Q <= 0.05 & LSAOutput$P <= 0.05,]

## Get sample names linked with numerical IDs to update eLSA formatted data.
names <- LSAInput[c(1:2)]
## Change the numerical IDs with the OTU data in order to make it more meaningful.
LSASubset$X <- names$V2[match(LSASubset$X, names$V1)]
LSASubset$Y <- names$V2[match(LSASubset$Y, names$V1)]
